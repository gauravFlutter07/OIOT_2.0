...
class PrettyDioLogger extends Interceptor {
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          ...
          _printPrettyMap(value, initialtabs: tabs);
        }
        ...
        if (compact && _canFlattenList(value)) {
          ...
          logPrint('║${_indent(tabs)} "$key": [');
          _printList(value, initialtabs: tabs);
  ...
  void _printList(List list, {int initialtabs = initialTab}) {
    int tabsCount = 2;
    list.asMap().forEach((i, dynamic e) {
      final isLast = i == list.length - 1;
      if (e is Map) {
        if (compact && _canFlattenMap(e)) {
          logPrint('║${_indent(initialtabs)}  $e${!isLast ? ',' : ''}');
        } else {
          _printPrettyMap(e, initialtabs: initialtabs + 1, isListItem: true, isLast: isLast);
        }
      } else {
        logPrint('║${_indent(initialtabs + tabsCount)} $e${isLast ? '' : ','}');
      }
    });
  }
